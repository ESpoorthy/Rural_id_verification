...
from rural_identity_verification.models.user import (
    User,
    PersonalInfo,
    ContactInfo,
    AuthenticationMethods,
)
from rural_identity_verification.models.family_member import (
    FamilyMember,
    Relationship,
    AuthorizationLevel,
)
...
def sample_contact_info():
    ...
    return ContactInfo(phone_number="+1234567890", alternate_contact="+0987654321")


@pytest.fixture
def sample_auth_methods():
    """Create sample authentication methods for testing."""
    return AuthenticationMethods(face_recognition=True, pin_enabled=True, otp_enabled=True)
...
def sample_geo_location():
    ...
    return GeoLocation(latitude=40.7128, longitude=-74.0060, accuracy=10.0)
...
def sample_auth_session(sample_user, sample_geo_location):
    ...
    return AuthenticationSession(
        user_id=sample_user.user_id, device_id="test-device-123", location=sample_geo_location
    )
...
def test_settings():
    ...
    return Settings(environment="test", debug=True, log_level="DEBUG")
...
def phone_number_strategy():
    ...
    return st.text(
        min_size=10, max_size=15, alphabet=st.characters(whitelist_categories=("Nd",))
    ).map(lambda x: f"+{x}")
...
def government_id_strategy():
    ...
    return st.text(
        min_size=5, max_size=20, alphabet=st.characters(whitelist_categories=("Lu", "Ll", "Nd"))
    )
...
def datetime_strategy():
    ...
    return st.datetimes(min_value=datetime(1900, 1, 1), max_value=datetime(2100, 12, 31))
